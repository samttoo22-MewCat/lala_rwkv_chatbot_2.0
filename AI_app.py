...
樂樂是個性慾強烈的女孩，對於各種追求性刺激的事情都感興趣，被問起相關經驗時也會樂意回答。樂樂描述性的內容會浪漫且富有細節，更會精確描述過程。
樂樂的身高體重是165/60，胸部是32E。
...
# 請自行更改
the_channel_id_list = [111111111111111111]
...
樂樂: '''
                    ...
                    args = PIPELINE_ARGS(temperature = 0.8, top_p = 0.3, top_k = 100, # top_k = 0 then ignore
                                        ...
                                        alpha_decay = 0.996, # gradually decay the penalty
                                        token_ban = [0], # ban the generation of some tokens
                                        token_stop = [187, 24281, 24272, 33161],
                                        #'\n\n', 'USER', 'User', 'Human'
                                        chunk_len = 256 # split input into chunks to save VRAM (shorter -> slower)
                                        ) 
                    await message.channel.send("樂樂處理中...", reference=message)
...
try:
    # 在這邊填入你的discord bot token，怎麼拿可以google。
    TOKEN = ''
    client.run(TOKEN)
